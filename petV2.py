...
button12325 = telebot.types.KeyboardButton(text='играть')
...
def edatick():
    global eda
    if eda <= 20:
        Bot.send_message(chat.id, "Я проголодался, покорми меня!")
    with lock:
        if eda > 0:
            eda = max(0, eda - 4)
...
def vodatick():
    global voda
    if voda <= 20:
        Bot.send_message(chat.id, "Я хочу пить!")
    with lock:
        if voda > 0:
            voda = max(0, voda - 5)
...
def happytick():
    global happy
    if happy <= 20:
        Bot.send_message(chat.id, "Что то мне грустно....")
...
def powertick():
    ...
    if power <= 20:
        Bot.send_message(users[0], "Я устал, хотелось бы поспать....")
    ...
    with lock:
        ...
        if power > 0:
            timer = threading.Timer(60.0, powertick)
            timer.start()
powertick()
    ...
    if namepet == '':
        ...
        if coudown == 0:
            ...
            if coudowneda == 0:
                ...
                Bot.send_message(message.chat.id, "Питомец покормлен, следующий раз через 30 минут")
                edatick()
                coudownedatick()
            else:
        ...
        if coudown == 0:
            ...
            if coudownvoda == 0:
                ...
                Bot.send_message(message.chat.id, "Питомец напоен, следующий раз через 30 минут")
                vodatick()
                coudownvodatick()
        ...
        if coudown == 0:
            ...
            if power > 0:
                ...
                happy += 10
                happytick()
                Bot.send_message(message.chat.id, "Вы погладили питомца. Питомец доволен. Великий Китайский партия тоже")
            else:
        ...
        if coudown == 0:
            ...
            if power > 0:
                ...
                happy += 30
                happytick()
                Bot.send_message(message.chat.id, "Вы поигрались с питомцем")
            else:
                Bot.send_message(message.chat.id, "Питомец измотан.")
        else:
            Bot.send_message(message.chat.id, f'Питомец спит, попробуй через {coudown // 60} минут')
